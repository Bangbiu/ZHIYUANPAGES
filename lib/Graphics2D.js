...
export const POLY = {
    ...
    "button": [[0,0],[200,0],[200,100],[0,100]],
...
export class GraphicalRect extends DOMRect {
     ...
     */
    getCenter() {
        return new Vector2D(this.left + this.width/2, this.top + this.height/2);
    }
...
export class Graphics2D {
     ...
     */
    calculateBound(scale=Graphics2D.DEF_SCALE,ctx=undefined) {
        let pen = new Vector2D(0.0,0.0);
        ...
        this.path.forEach(cmd => {
            let params = cmd[1];
            switch (cmd[0]) {
                case "M": { pen.moveTo(params[0],params[1]).scale(scale);break; }
                case "L": { pen.moveTo(params[0],params[1]).scale(scale);break; } 
                case "H": { pen.setX(params[0]).scaleXY(scale.x,1);break; }
                case "V": { pen.setY(params[0]).scaleXY(1,scale.y);break; }
                case "Q":  { 
...
export class GraphicsText extends Graphics2D {
     ...
     * @param {Vector2D} scale
     */
    tracePath(ctx,scale=undefined) {
        this.calculateBound(undefined, ctx);
        ctx.moveTo(this.bound.left,this.bound.top);
        ctx.lineTo(this.bound.right,this.bound.top);
        ctx.lineTo(this.bound.right,this.bound.bottom);
        ctx.lineTo(this.bound.left,this.bound.bottom);
        ctx.lineTo(this.bound.left,this.bound.top);
    }
     ...
     * @returns {GraphicalRect}
     */
     calculateBound(scale=undefined,ctx) {
        let mat = ctx.measureText(this.text);
        let height = (mat.actualBoundingBoxAscent + mat.actualBoundingBoxDescent);
        //console.log(height);
        this.bound = new GraphicalRect(-mat.width/2,-height/2,mat.width,height);
        return this.bound;
     }
