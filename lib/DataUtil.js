...
class SObject {
    ...
    /**
     * 
     * @param {SObject} values
     * @returns {SObject}
     */
    insertValues(values) {
        return SObject.insertValues(this,values);
    }
    ...
    static initialze(target,values,assign = SObject.DATA_COPY) {
    ...
    static setValues(target,values,assign = SObject.DATA_COPY) {
        for (const key in values) {
            target.assign(key,values[key],assign);
        }
        return target;
    }
    ...
    static copy(target,source) {
        if (source == undefined) 
            return SObject.copyData(target);
        else   
    ...
    static clone(target,source) {
        if (source == undefined) {
            return SObject.cloneData(target);
        } else {    
