...
class SObject {
     ...
     */
    initialize( values, def, assign=SObject.DATA_COPY ) {
     ...
     * @returns {SObject}
     */
    setValues( values = {}, assign = SObject.DATA_COPY ) {
     ...
     * @returns {SObject}
     */
    updateValues( values = {}, assign = SObject.DATA_COPY ) {
     ...
     * @returns {SObject}
     */
    insertValues( values = {}, assign = SObject.DATA_COPY ) {
    ...
    copy(source = undefined) {
        return SObject.copy(this,source)
    }
    ...
    clone(source=undefined) {
        return SObject.clone(this,source);
    }
     ...
     */
    assign(key,value,type) {
        SObject.assign(this,key,value,type);
    }
     ...
     * @param {TU.DataAssignType} assign 
     */
    static initialize(target,values,def,assign = SObject.DATA_COPY) {
        SObject.setValues(target,def,assign);
     ...
     * @returns {SObject}
     */
    static setValues(target,values,assign = SObject.DATA_COPY) {
     ...
     * @returns {SObject}
     */
    static updateValues(target,values,assign = SObject.DATA_COPY) {
        for (const key in values) {
            if (key in target)
                SObject.assign(target,key,values[key],assign);
        }
     ...
     * @returns {Object}
     */
    static insertValues(target, values, assign = SObject.DATA_COPY) {
        for (const key in values) {
            if (!(key in target)) 
                SObject.assign(target,key,values[key],assign);
        }
     ...
     * @param {TU.DataAssignType} type 
     */
    static assign(target,key,value,type) {
        switch (type) {
            case SObject.DATA_COPY: {
                target[key] = SObject.copyData(value);
                break;
            }
            case SObject.DATA_CLONE: {
                target[key] = SObject.cloneData(value);
                break;
            }
    ...
    static copy(target,source) {
        if (source == undefined) 
            return Object.assign(Object.create(target.constructor.prototype),target);
        else   
            SObject.setValues(target,source,SObject.DATA_COPY);  
        return target;
    }
    ...
    static copyData(data) {
        switch (typeof data) {
            case "object": {
                if (data instanceof SObject) {
                    return data.copy();
                } else if (data instanceof Array){
                    return [...data];
                } else {
                    return Object.assign(Object.create(data.constructor.prototype),data);
                }
            }
            default: { return data; }
        }        
     ...
     * @returns {SObject}
     */
    static clone(target,source) {
        if (source == undefined) {
            const res = Object.create(target.constructor.prototype);
            for(const key in target) {
                SObject.assign(res,key,target[key],SObject.DATA_CLONE);
            }
            return res;
        } else {    
            Object.assign(target, SObject.cloneData(source));
            return target;
        }
    }
            ...
            case "object": {
                if (data instanceof SObject) {
                    ...
                    data.forEach(element => {
                        res.push(SObject.cloneData(element));
                    });
                    return res;
                } else {
                    const res = Object.create(data.constructor.prototype);
                    for(const key in data) 
                        res[key] = SObject.cloneData(data[key]);
                    return res;
                }
            }
            default: { return data; }
