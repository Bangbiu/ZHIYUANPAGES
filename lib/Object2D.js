...
import { GraphicalRect, Graphics2D, GraphicsText } from "./Graphics2D.js";
...
class ContextTransf extends SObject {
    ...
    constructor( parameters = {} , assign = SObject.DATA_CLONE) {
        ...
        if (!(this.trans instanceof Vector2D)) this.trans = new Vector2D(this.trans);
        if (!(this.rot instanceof Rotation2D)) this.rot = new Rotation2D(this.rot);
        if (!(this.scale instanceof Vector2D)) this.scale = new Vector2D(this.scale);
    }
...
class Object2D extends SObject {
    ...
    initialize( parameters = {} , def = Object2D.DEF_PROP, assign = SObject.DATA_IDEN) {
        ...
        //Set Default Values
        this.setValues(def,SObject.DATA_CLONE);

        //Parse Parameter
        SObject.resolve(parameters,"pos",Vector2D);
        SObject.resolve(parameters,"rot",Rotation2D);
        SObject.resolve(parameters,"scale",Vector2D);
        SObject.resolve(parameters,"stret",Vector2D);
        SObject.resolve(parameters,"graphics",Graphics2D);

        SObject.resolve(parameters,"fillColor",Color);
        SObject.resolve(parameters,"borderColor",Color);
        SObject.resolve(parameters,"emissiveColor",Color);
...
class MouseEventMap extends SObject {
    ...
    clone(actor=this.#actor) {
        return new MouseEventMap(actor).setValues(this,SObject.DATA_IDEN);
    }
