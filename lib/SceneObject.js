...
export class SceneInteractive extends SceneObject {
    ...
    onMouseMove(event) {
        if (this.ctrlPt != undefined && event.info.buttons > 0 && this.draggable) {
            this.pos.copy(this.ctrlPt).reverse().add(event.mCtxPos);
        }
    }
     ...
     * @param {ContextMouseEvent | MouseEvent} event 
     * @param {CanvasRenderingContext2D} ctx 
     */
    updateMouseInfo(ctx=SceneObject.DefaultContext,event) {
        if (event instanceof MouseEvent) 
            event = new ContextMouseEvent(event,DEF_TRANSF);

        switch (event.info.type) {
            case "mousedown": { 
                if (this.isMouseIn) this.onMouseDown(event); 
                break; 
            }
            case "mouseup" : { 
                if (this.isMouseIn) this.onMouseUp(event); 
                break;
            }
            ...
            case "mousemove" : {
                ...
                if (this.isMouseIn) this.onMouseMove(event); 
                break;
            } 

            case "wheel" : {
                if (event.info.altKey && this.isMouseIn) {
        ...
        if (this.isMouseIn) {
            this.innerTransf.transform(ctx);
            this.innerTransf.apply(event.mCtxPos);
            this.components.forEach(comp => {
     ...
     * @returns {[MouseEvent,Vector2D,Vector2D]}
     */
    static parseMouseEvent(event,transf=DEF_TRANSF) {
        let box = /** @type {HTMLCanvasElement} */ (event.target).getBoundingClientRect();
        let pos = new Vector2D(event.clientX-box.left, event.clientY-box.top);
        return [event,pos,transf.apply(pos.copy())];
    }
